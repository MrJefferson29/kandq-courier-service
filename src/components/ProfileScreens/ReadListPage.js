...
const ReadListPage = () => {
  ...
  const getUserReadingList = async () => {
    ...
    try {
      const { data } = await (
        await axios.get(`https://delivery-wheat.vercel.app/user/readList`, config, { withCredentials: true },)
      ).data;
      setReadList(data);
      setLoading(false);
    } catch (error) {
      ...
      {loading ? (
        ...
        <div className="Inclusive-readList-page">
          ...
          <div className="readList-top-block">
            <img
              src={`https://delivery-wheat.vercel.app/userPhotos/${activeUser?.photo}`}
